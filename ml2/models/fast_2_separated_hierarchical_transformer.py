...
def create_model(params, training, attn_weights=False):
    ...
    assert len(params["params_sep_local"]) == 2

    params["return_attn_weights"] = attn_weights
    hierarchical_transformer = Fast2SeparatedHierarchicalTransformer(params)

    params["return_attn_weights"] = attn_weights
    input1 = tf.keras.layers.Input((None, None), dtype=tf.int32, name="input1")
    input2 = tf.keras.layers.Input((None, None), dtype=tf.int32, name="input2")
    transformer_inputs = {"input1": input1, "input2": input2}
    model_inputs = [input1, input2]

    positional_encoding1 = tf.keras.layers.Input(
        (None, None, None), dtype=tf.float32, name="positional_encoding1"
    )
    positional_encoding2 = tf.keras.layers.Input(
        (None, None, None), dtype=tf.float32, name="positional_encoding2"
    )
    transformer_inputs["positional_encoding1"] = positional_encoding1
    transformer_inputs["positional_encoding2"] = positional_encoding2
    model_inputs.append(positional_encoding1)
    model_inputs.append(positional_encoding2)
...
class Fast2SeparatedHierarchicalTransformer(tf.keras.Model):
    ...
    def call(self, inputs, training):
        ...
        Args:
            inputs: dictionary that contains the following (optional) keys:
                input1: int tensor with shape (batch_size, dim1.1, dim1.2), dim1.1 and dim1.2 are defined in params['params_sep_local']
                input2: int tensor with shape (batch_size, dim2.1, dim2.2), dim2.1 and dim2.2 are defined in params['params_sep_local']
                positional_encoding1: float tensor with shape (batch_size, dim1.1, dim1.2, d_embed_enc), dim1.1 and dim1.2 are defined in params['params_sep_local'], custom positional encoding for input1
                positional_encoding2: float tensor with shape (batch_size, dim2.1, dim2.2, d_embed_enc), dim2.1 and dim2.2 are defined in params['params_sep_local'], custom positional encoding for input2
        ...
        custom_positional_encoding1 = inputs["positional_encoding1"]
        custom_positional_encoding2 = inputs["positional_encoding2"]
        ...
        (
            ...
            input1, input2, custom_positional_encoding1, custom_positional_encoding2, training
        )
    ...
    def encode(self, input1, input2, positional_encoding1, positional_encoding2, training):
        """
        Args:
            input1: int tensor with shape (batch_size, dim1.1, dim1.2), dim1.1 and dim1.2 are defined in params['params_sep_local']
            input2: int tensor with shape (batch_size, dim2.1, dim2.2), dim2.1 and dim2.2 are defined in params['params_sep_local']
        ...
        custom_positional_encodings = [positional_encoding1, positional_encoding2]
        ...
        for i in range(len(inputs)):
            ...
            if self.params["fix_local_embed"]:
