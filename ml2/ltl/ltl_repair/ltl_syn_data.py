...
from ...globals import LTL_REP_ALIASES, LTL_REP_BUCKET_DIR, LTL_REP_WANDB_PROJECT
...
class LTLSynData(SupervisedData):
    ...
    def input_generator(self):
        for inp, _, _ in self.generator():
            yield inp

    def target_generator(self):
        for _, tar, _ in self.generator():
            yield tar

    def tf_generator(self):
        for inp, tar, _ in self.generator():
...
class LTLSynSplitData(SplitSupervisedData):
    ...
    def load_from_path(cls, path: str, splits: list = None):
        ...
        if path[path.rindex("/") + 1 :] == "scpa-2":  # hack for additional split
            splits.append("test_fixed")
